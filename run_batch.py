...
DATAFORSEO_ENDPOINT = "/v3/serp/google/organic/live/advanced"
    ...
    try:
        ...
        for item in items:
            if item.get("type") == "ai_overview":
                # Texto del AI Overview — puede estar en distintos campos
                ai_text = (item.get("text", "") or
                           item.get("description", "") or
                           item.get("featured_title", "") or "")

                # Citaciones — pueden estar en references, items o asistant_message
                refs = (item.get("references") or
                        item.get("items") or
                        item.get("asistant_message", {}).get("references", []) or [])

                for ref in refs:
                    domain = ""
                    source_url = ref.get("url", ref.get("source", ""))
                    if source_url:
                        try:
                ...
                # Si no hay texto pero hay items anidados, intentar extraer de ahí
                if not ai_text and item.get("items"):
                    for sub in item.get("items", []):
                        ai_text += sub.get("text", "") or sub.get("description", "") or ""
